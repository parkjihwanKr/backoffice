...
public class Scheduler {
    ...
    public void updateRemainingVacationDaysMonthly() {
        membersService.updateOneForRemainingVacationDays(ScheduledEventType.MONTHLY_UPDATE);
    }
            ...
            for(Members unCompletedEvaluationMember : unCompletedEvaluationMemberList){
                ...
                notificationsService.generateEntityAndSendMessage(
                        notificationsService.toNotificationData(
                                unCompletedEvaluationMember, admin,
    ...
    public void configureVacationRequestPeriod() {
        ...
        LocalDateTime firstDayOfMonth = DateTimeUtils.withDayOfMonth(1);
        LocalDateTime firstMonday = DateTimeUtils.findFirstMonday(firstDayOfMonth);
        ...
        // 로그 확인
        String message = "새 휴가 신청 기간 설정됨: " + startDate + " ~ " + endDate;
        log.info(message);
        Members admin = membersService.findHRManager();
        if(admin == null){
            admin = membersService.findCeo();
